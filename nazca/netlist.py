...
def interconnect_logger(msg='', level='info'):
    ...
    fullmsg = f"IC-{cfg.interconnect_msgcnt}: {msg}"
...
def pin_drc(pin1, pin2, t):
    ...
    if DRCxsection or DRCangle or DRCwidth or DRCsymmetry:
        ...
        if DRCsymmetry:
            drc_cnt += 1
            msg = "DRC-{}: symmetry mismatch in cell '{}' for xs = {} on pins {} @ {}".\
...
def validate_basename(basename):
    ...
    for bname in cfg.basenames:
        ...
        if bname[0:size] == basename[0:size]:
            logger.error("basename overlap {} - {}".format(bname, basename))
            return False
    return True
        ...
        if namesin is None:
            ...
            for namein, nameout in zip(namesin, namesout):
                if namein in self.pin.keys():
                    ...
                    logger.warning("'raise_pins': pin '{}' not defined.".\
                        format(namein))
...
class Cell():
    ...
    def __init__(self, name='cell', celltype='element', instantiate=True,
            autobbox=False, hull=None, store_pins=None, hashme=False, cnt=False,
            params=False, show_hull=False, hull_based_bbox=None):
        #TODO: params unused?
        ...
        if name in cfg.cellnames.keys():
            ...
            if instantiate is True:
                logger.warning("Duplicate cell name in nd.Cell(name='{}') renamed to '{}'.".\
                    format(name, name2))
            name = name2
        else:
        ...
        self.ranges = {} #  for storing paramter information like the allowed range
        self.bbox = None
    ...
    def add_property(self, item):
        ...
        def merge_properties(D1, D2):
            ...
            if not D1keys or not D2keys:
                logger.warning("In cell '{}': Overwriting properties: {} to {}".\
                    format(self.cell_name, D1, D2))
                return D2
            else:
    ...
    def _copy_cell2instance(self, parent_cnode, flip, flop, array=None, scale=1.0):
        ...
        """
        instance = Instance(name="I_{}".format(self.cell_name))
        ...
        if array is not None:
            try:
                ...
                if not self.instantiate:
                    logger.warning(
"Trying to set array on cell '{}' with instantiate=False"\
" in parent cell '{}'."\
" Only a single instance will be visible."\
" Set instantiate=True to obtain the full array in GDS.".\
                    format(self.cell_name, parent_cnode.cell.cell_name))
                    array = None
            except Exception as excp:
                logger.exception("Error: incompatible array format {} when placing "\
                    "cell '{}' in '{}'. Array will be ignored.\n{}".\
                    format(array, self.cell_name, parent_cnode.cell.cell_name, excp))
                array = None
    ...
    def put(self, *args, flip=False, flop=False, array=None, scale=1.0,
        ...
        if self.cnode is parent_cnode:
            logger.error('can not connect cell \'{}\' to itself.'.\
...
#                            print('    other', pp[0].name)
        ...
        if self.pin2 is not None and not cfg.group_connect:
            if flip:
                cfg.netlist_errcnt += 1
                msg = "NET-{} can not close the p2p connection with flip is True in cell '{}'.".\
                    format(cfg.netlist_errcnt, self.cell_name)
                logger.error(msg)
                netlist_exception(msg)
            elif C[0] is not None:
                cfg.netlist_errcnt += 1
                msg = "NET-{} can not close the p2p connection with non-empty put() call on cell '{}'.".\
                    format(cfg.netlist_errcnt, self.cell_name)
                logger.error(msg)
                netlist_exception(msg)
            else:
...
#                        f" {inode.instance.name:20}"\
    ...
    def _calculate_bbox(self, instance=True):
        ...
        if self.use_hull: # make a hull for this cell (self)
            try: # ConvexHull may fail
                ...
                self.hull = None
                hull_created = False
                logger.exception("No hull resolved in cell '{}'. Using square bbox instead.\n{e}".\
                    format(self.cell_name), excp) # probably 1D polygons, like strt(length=0)
        ...
        if draw_bbox:
            if len(self.gdsfiles) > 0:
                if self.bbox is None:
                    logger.warning("Can not determine a bounding box for cell '{}',".format(self.cell_name))
                    logger.warning("  e.g. due to only non-native GDS subcell(s) and/or empty subcell(s).")
                    logger.warning("  In case of gds loading causing this message:")
                    logger.warning("  Try keyword native=True in  to get the complete bbox")
                    logger.warning("  or set keyword bbox=False to get rid of this message.")
                else:
                    logger.warning("Not all instances in cell '{}' have a known bbox,".format(self.cell_name))
                    logger.warning("  e.g. due to a non-native GDS subcell or an empty subcell.")
                    logger.warning("  Therefore, the bbox maybe smaller than the actual structures.")
                    logger.warning("  Try option native=True in gds loading to get the complete bbox")
                    logger.warning("  or set bbox=False to get rid of this message.")
            if self.bbox is not None:
    ...
    def close(self):
        ...
        """
        if self.closed:
            logger.error('Cell \'{}\' already closed.'.format(self.cell_name))
        ...
        if not self.add_bbox_flag:
            ...
            if self.params:
                bbu.parameters(parameters=self.parameters)
        ...
        if cfg.store_bbname and self.version is not None:
            if self.instantiate:
                bbtxt = "cellname: {}".format(self.cell_basename)
                if isinstance(self.version, dict):
                    pdk = self.version.get('pdk_version', None)
                    owner = self.version.get('owner', None)
                    if pdk is not None:
                        bbtxt += "\npdk_version: {}".format(pdk)
                    if owner is not None:
                        bbtxt += "\nowner: {}".format(owner)
                Annotation(text=bbtxt, layer=get_layer('bb_name')).put(0)
            else:
                raise Exception("Cell must be intantiated to add a name and version to it"\
                    " Cell: '{}'".format(self.cell_name))
                ...
                if XS is not None:
                    ...
                    if layer is not None: # add a new layer (only for new pinstyles)
                        ...
                        if presentstyle is None:
                            ...
                            logger.warning(f"In setting pin '{cfg.cells[-1].cell_name}'.'{self.name}', can not add layer {layer} to existing pinstyle '{xs}' having already layer {presentstyle['layer']}. "
    ...
    def __init__(self, points=None, layer=None):
        ...
        if cfg.use_hull:
            try:
                ...
                logger.error("CovexHull Error Polygon: {}\n{}ting hull=False. Set".format(excp, points))
                hull = False
        if not hull:
            xy = list(zip(*points))
            self.bbox = (min(xy[0]), min(xy[1]), max(xy[0]), max(xy[1]))
...
#        cellsnotreused = list(notused)
    ...
    if gds_elements_unknown and cfg.show_unknown_GDS_records:
        ...
        logger.warning("gds2nazca: Ignoring elements in cell (branch) '{}': {}".\
        ...
        for name in sorted(names):
            ...
            if (cellmap[name] in cfg.cellnames.keys()) and not native: # native solves the conflicts itself
                if cellmap[name] is not name:
                    ...
                    rename = ''
                if reuse:
                    logger.warning("use_first=True in load_gds:"\
                        " from file '{}'".format(name, filename))
                else:
                    raise Exception(
"""
        ...
        for nextnode, mat in neighbours:
            if nextnode not in self.nodes_visited:
                ...
                if (nextnode.cnode is _cnode0):
                    ...
                    logger.warning("Skipping node {} in cell '{}': "\
...
# =============================================================================
    ...
    def celltree_iter(
        ...
        for cnode, level, org, flip, scale in cell_iter:
            ...
            if not makecell or not visited: # create new cell or recreate flat cell:
                if infolevel > 0:
