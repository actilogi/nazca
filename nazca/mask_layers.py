...
def add_layer(\
    ...
    if unknown:
        ...
        #   - If a layer is redirected, raise a warning (first time it occurs)
        isDefault = False
        if name in cfg.default_layers.keys():
            LDT_or_name = cfg.default_layers[name]
            if isinstance(LDT_or_name, str):
        ...
        if not isDefault:
            ...
            if cfg.redirect_unknown_layers: # do not auto-create unknown layers.
                if layer not in unknown_layers:
                    ...
                    logger.warning("Unknown layer {} in get_layer."\
...
def get_layer(layer):
    ...
    Returns:
        str: layer_ID (layer_name)
    """
    if isinstance(layer, str) or layer is None:
        if layer in cfg.layername2LDT.keys():
            return layer
        else:
    ...
    if status > 1:
        if LDT not in cfg.LDT2layername.keys():
            ...
            logger.exception("Non-unique layer identifier used: {0}. "\
