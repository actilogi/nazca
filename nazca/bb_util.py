...
# Put here to avoid loading utils.py:
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if funcid != cfg.hash_func_id:
                    logger.warning("Reusing a basename across functions: '{}'".
                        format(basename))
                    #raise
            except KeyError:
                logger.debug('Adding basename {} to cfg.basenames'.
                        format(basename))
...
def bbox_layers_ignore(layers):
     """Set the layers that should be ignore in bbox calculations."""
     cfg.bbox_layers_ignore = [nd.get_layer(ml) for ml in layers]
...
def rangecheck(allowed_values):
    ...
    for varname, values in allowed_values.items():
        ...
        #if low <= value <= high:
        #    continue
        elif values['default'] < values['min']:
    ...
    if styledict is not None:
        for item, setting in styledict.items():
            if item in cfg.pinstylelabels:
                ...
                logger.warning("No valid pinstyle key  '{}' for pinstyle. Available keys: {cfg.pinstylelabels}".\
                    format(item, list(keys)))
...
def make_pincell(layer=None, shape=None, size=None, style=None):
    ...
    if shape not in cfg.pinshapes.keys():
        ...
        mes += "Fall back type '{}' will be used.".format(shape_default)
        logger.warning(mes)
        shape = shape_default
        ...
        try:
            ...
            if stubcell is not None: # use existing Nazca cell for stub+pin
                ...
                if node.xs is None: # pin cell only, no stub
                    ...
                    xs = node.xs
                    xsstyle = nd.get_xsection(xs).pinstyle
                    if xsstyle in pinstylenames and pinstyle is None:
                        stylestr = xsstyle
                ...
                if xs is None: # pin only
                    arrow = make_pincell(layer=pinlayer, shape=pinshape,
                        size=pinsize, style=stylestr)
                    arrow.put(node)
                else: # stub
                    if length is not None:
                        ...
                        if stubstyle is None:
                            stubstyle = 'default'
                        length = nd.cfg.pinstyles[stubstyle].get('stub_length', 0)
                    name = _makestub(
                        xs_guide=xs,
                        width=width,
                        length=length,
