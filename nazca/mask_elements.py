...
"""
This module defines mask elements such as straight and bent waveguides
...
from math import sin, cos, acos, atan2, degrees, radians, pi, sqrt, hypot, copysign
...
from .generic_bend import curve2polyline, gb_point, gb_coefficients, sinebend_point
...
def Tp_straight(length=10, width=1.0, xs=None, layer=None,
        edge1=None, edge2=None, edgepoints=50, name=None):
    ...
    def cell(length=length, width=width, xs=xs, layer=layer,
            edge1=edge1, edge2=edge2, edgepoints=edgepoints,
            name=name):
        ...
        with Cell(name=name, cnt=True) as C:
            ...
            for lay, grow, acc, polyline in layeriter(xs, layer):
                ...
                if not polyline:
                    if edge1 is None:
                        outline = [
                            (0-c1, width*a1+b1), (length+c2, width*a1+b1),
                            (length+c2, width*a2+b2), (0-c1, width*a2+b2)
                        ]
                    else:
                        if edge2 is None:
                            edge2 = edge1
        ...
        C.properties['parameters'] = {
              ...
              'call':   {'value': 'strt', 'unit': None, 'type': 'str'},
         }
        return C
    return cell
...
def Tp_taper(length=100, width1=2, width2=3, xs=None, layer=None, name=None):
    ...
    def cell(length=length, width1=width1, width2=width2, xs=xs, layer=layer,
        ...
        C.properties['parameters'] = {
            ...
            'xs':     {'value': xs, 'unit': None, 'type': 'str'},
            'call':   {'value': 'taper', 'unit': None, 'type': 'str'},
        }
        return C
    return cell
...
def Tp_ptaper(length=100, width1=1.0, width2=3.0, xs=None, layer=None,
    ...
    def cell(length=length, width1=width1, width2=width2, xs=xs, layer=layer,
        ...
        C.properties['parameters'] = {
            ...
            'width2': {'value': width2, 'unit': 'um', 'type': 'float'},
            'xs':     {'value': xs, 'unit': None, 'type': 'str'},
            'call':   {'value': 'ptaper', 'unit': None, 'type': 'str'},
        }
        return C
    return cell
    ...
    def cell(radius=radius, width=width, angle=angle, xs=xs, layer=layer,
        ...
        with Cell(name=name, cnt=True) as C:
            ...
            for lay, grow, acc, polyline in layeriter(xs, layer):
                ...
                if Ri < 0:
                    if Ri < -1e-6:
                        logger.warning("Side wall arc radius too small in cell '{}': side_radius={:.3f}, radius={:.3f}, xs='{}', width={:.3f}, offset={:.3f}. Setting it to 0.".\
...
def Tp_sinecurve(width=1.0, distance=200, offset=20, xs=None, layer=None,
        name=None):
    ...
    def cell(width=width, distance=distance, offset=offset, xs=xs,
        ...
        with Cell(name=name, cnt=True) as C:
            ...
            for lay, grow, acc, polyline in layeriter(xs, layer):
                ...
                xy = nd.util.polyline2polygon_asym(xy, width, grow=grow)
        ...
        C.properties['parameters'] = {
            ...
            'xs':     {'value': xs, 'unit': None, 'type': 'str'},
            'call':   {'value': 'sinecurve', 'unit': None, 'type': 'str'},
        }
    return cell


def Tp_cobra(xya=(100, 100, 10), width1=1.0, width2=1.0, radius1=0, radius2=0,
        offset1=None, offset2=None, xs=None, layer=None, name=None):
    ...
    """
    # TODO: add growy
    def cell(xya=xya, width1=width1, width2=width2, radius1=radius1,
            radius2=radius2, offset1=offset1, offset2=offset2, xs=xs,
            layer=layer, name=name):
        ...
        with Cell(name=name, cnt=True) as C:
            C.instantiate = cfg.instantiate_mask_element
            p1 = nd.Pin(name='a0', io=0, width=width1, xs=xs, show=True).\
                put(0, -offset1, 180)
            xya = (xya[0], xya[1]-offset1, xya[2])
            p2 = nd.Pin(name='b0', io=1, width=width2, xs=xs, show=True).put(*xya)
            ...
            for lay, grow, acc, polyline in layeriter(xs, layer):
                ...
                if callable(width1):
                    def wfun(t):
                        return width1(t)
                    xy2 = nd.util.polyline2polygon_asym(
                        xy,
                        wfun,
                        grow=grow
                    )
...
def Tp_euler(width1=1.0, width2=1.0, radius=50, angle=90,
        xs=None, layer=None, name='euler'):
    ...
    """
    def cell(width1=width1, width2=width2, radius=radius, angle=angle,
        ...
        with nd.Cell(name=name, cnt=True) as C:
            C.instantiate = cfg.instantiate_mask_element
            arad = radians(angle)
            ta = sqrt(2.0*abs(arad)/pi)
            N0 = 50
            t = linspace(0, ta, N0)
            t = t[-3:-1]
            y, x = fresnel(t)
            scale = abs(pi*ta*radius)

            if scale == 0:
                return C

            ds = scale*hypot(x[-1]-x[-2], y[-1]-y[-2])
            ...
            for lay, grow, acc, polyline in layeriter(xs, layer):
                ...
                shape = nd.util.polyline2polygon_asym(
                    xy=points,
                    width1=width1,
                    width2=width2,
                    grow=grow,
...
def Tp_viper(x, y, w, width1=None, width2=None,
       xs=None, layer=None, N=200, epsilon=1e-6, name='viper', params={}, **kwargs):
    """Template for a specific Viper implementation.

    Free parameter in function x, y, and w are provides via ** kwarg. Note that
    when using Tp_Viper all free parameters used in x, y, and w must be provided.
    ...
    def viper_cell(width1=width10, width2=width20, xs=xs, layer=layer, N=N, **kwargs):
        ...
        with nd.Cell(name=name, cnt=True) as C:
            for lay, grow, acc, line in nd.layeriter(xs, layer):
                ...
                for i in range(N):
                    ...
                    xy.append((X(t), Y(t)))
                    width.append(W(t))
                points = nd.util.polyline2polygon_asym(
                    xy,
                    width1=width,
                    anglei=180+aa,
                    angleo=ab,
