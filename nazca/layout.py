...
#        return self._newtree
    ...
    def setlayermap(self, layermap=None, mode="all"):
        ...
        if layermap:
            for layerold, layernew in layermap.items():
                ...
                if layerold in cfg.layername2LDT.keys():
                    if layernew is None:
                        self.layermap[layerold] = None
                    else:
                        layer_new = get_layer(layernew)
                        if layer_new not in cfg.layername2LDT.keys():
                            logger.warning(
                        ...
                        self.layermap[layerold] = get_layer(layernew)
    ...
    def export_topcells(self, topcells):
        ...
        if self.gds:  # save all external GDS file based instances
            ...
            if self.ascii:
                ga = gstream.GDSII_stream(os.path.join(self.dirname, GDS.filename))
...
def export(
    ...
    if not exported:
        msg = "No export type defined: plt, gds, or svg."
        logger.error(msg)
...
def export_svg(topcells=None, title=None, path="", **kwargs):
    ...
    Returns:
        None"""
    export(topcells, plt=False, gds=False, svg=True, info=False, title=title, **kwargs)
