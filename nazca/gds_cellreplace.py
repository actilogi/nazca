...
# implementation.
#
# (c) 2016-2019 Katarzyna Lawniczuk, Ronald Broeke
#==============================================================================
...
def __exclude_substrings(allBlackNames):
...
def __findBlackCells(blackBasename, cells=[], excludes=None, infolevel=0):
...
def __readPcellParams(blackBasename, gdsin, allInputCellNames, excludes=None,
        infolevel=0):
    ...
    allCellsOf1type = __findBlackCells(blackBasename=blackBasename,
        cells=allInputCellNames, excludes=excludes, infolevel=infolevel)
    if infolevel == 2:
    ...
    for cellname in allCellsOf1type:
        ...
        for varname, valuestr in PcellStrParams.items():
            ...
            if valuestr.upper() == 'TRUE':
                ...
                logger.info('ERROR: parameter value not recognized: {}'.format(v))
...
def __createWhitePcellLib(gdsin, whitelibrary=None, black2whiteMap=None,
    ...
    allWhiteCells = []
    allWhiteCellNames = []
    excludes = __exclude_substrings(list(black2whiteMap.keys()))
    for blackBasename, whiteFunction in black2whiteMap.items():
        ...
        if whiteFunction is None:
            continue
        cells_x_Params = __readPcellParams(blackBasename=blackBasename,
            gdsin=gdsinstream, allInputCellNames=allInputCellNames,
            excludes=excludes, infolevel=infolevel)
        for cellname, parameters in cells_x_Params.items():
            try:
                ...
                logger.exception("White box function or whitbox function call.\n"\
...
def replaceCells(gdsin, gdsout=None, PcellFunctionMap=None,
        ScellMapping=None, md5=False, md5suffix='.md5', suffix='_white', addtime=True, infolevel=0):
