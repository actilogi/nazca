...
Copyright (c) Bright Photonics B.V.
"""
...
flip = 1
def find_optical_level(start, instance_stack=None, log=False, flip=1):
    """Find if a node has an optical edge. Drill down if needed/possible.

    This function finds if node <start> has an optical edge.
    If not, it is checked if node is connected to an instance to, if so,
    drill down into
    When drilling down it creates a "C2I_map" attribute in every Instance it
    passes to find the way back up from a Cell to the Instance along
    the celltree path it came from.

    Args:
        start (Node): a higher level Node from which you want to go down
        log (bool): prints al nodes it passes in an indented way if True

    Returns:
        Node, Node, list, float:
            cell_node (Node): start Node of optical connection,
    ...
    while True:
       ...
       # print(f"* cell_node:   {cell_node.xya()}")
        for nb, L, direction, path in cell_node.path_nb_iter(sigtype='dis'):
        ...
        #    print(f"  opt_position {posrel}")
            next_nodes_opt.append((nb, L, direction, posrel.xya(), path))
...
path = 0
def _pathfinder(start, end=None, log=False, logfilename='trace.log'):
    """Trace an optical connection by decending down into cells with optical neighbors.
    ...
    while not end:
        ...
        if not backtrack:
            PIN1, PIN2list, instance_stack, flip = find_optical_level(start, instance_stack, log, flip)
            ...
            pinid = (tuple(instance_stack), PIN1)
            if pinid in visited:
                if log:
                    F.write(f"{(count+1)*'  '}=== END PATH {path}: LOOP CLOSURE IN C-{PIN1.cnode.cell.id} on pin '{PIN1.name}'\n\n")
                paths[path] = opt_netlist
        ...
        opt_netlist.append((PIN1, PIN2, L, (point_old, point_new), line))
        ...
        while instance_stack:
            ...
            if nbs_side:
                ...
                paths[path] = opt_netlist
                paths_endpin[path] = (instance_stack, PIN2)
                path += 1
                if log:
...
def print_paths(paths, width, layer, stdout=True, endpins=None, pathfilename=None):
    ...
    for pathnum, stuff in paths.items():
        points = []
        Ltot = 0
        for i, (p1, p2, L, point, line) in enumerate(stuff):
            Ltot += L
            x0, y0 = point[0].xya()[:2]
            if i> 0:
