...
class Interconnect():
    ...
    def __init__(self, radius=None, width=None, angle=90, xs=None,
            layer=None, adapt_width=False, adapt_xs=False, instantiate=False,
            pinstyle=None, offset=None, varname=None, doc=''):
    ...
    def _p2p_parse(self, pin1, pin2=None, xs=None, width1=None, width2=None,
        ...
        # Keep the original p2p pins (identity) where possible to keep netlist connectivity
        # in the right cell scope.
    ...
    def strt(self, length=None, width=None, pin=None, xs=None, edge1=None,
            edge2=None, edgepoints=50, name=None, arrow=True):
        ...
        with nd.Cell('{}_{}'.format(name, xs), instantiate=self.instantiate, cnt=True) as ICcell:
            ...
            e1 = self.line(length=length, width=width, xs=xs, edge1=edge1,
                edge2=edge2, edgepoints=edgepoints).put(0)
    ...
    def bend(self, radius=None, angle=None, width=None, pin=None, xs=None,
        ...
        with nd.Cell('{}_{}'.format(name, xs), instantiate=self.instantiate, cnt=True) as ICcell:
            ICcell.group = 'interconnect'
            trace.trace_start()
            e1 = self._arc(radius=radius, width=width, angle=angle, xs=xs,
                offset=offset).put(0)
            nd.Pin('a0', io=0, width=width, xs=xs).put(e1.pin['a0'])
            nd.Pin('b0', io=1, width=width, xs=xs).put()
            if arrow:
    ...
    def strt_p2l(self, pin=None, ref=None, width=None, xs=None,
        ...
        if ref is None:
            ...
            msg = "IC-{} strt_p2l needs a reference line.".format(cfg.interconnect_errcnt)
    ...
    def _sbend_solve(self, radius=None, width=None, pin=None, xs=None, offset=20,
            Ltot=None, length1=None, length2=None):
    ...
    def sbend(self, radius=None, width=None, pin=None, xs=None, offset=20,
            Ltot=None, length1=None, length2=None, name=None, arrow=True):
        ...
        parse, result = self._sbend_solve(xs=xs, width=width, radius=radius,
            pin=pin, offset=offset, Ltot=Ltot, length1=length1, length2=length2)
        ...
        with nd.Cell('{}_{}'.format(name, xs), instantiate=self.instantiate, cnt=True) as ICcell:
            ICcell.group = 'interconnect'
            trace.trace_start()
            e1 = self.line(La, width, xs=xs).put(0)
    ...
    def _sbend_p2p_solve(self, pin1=None, pin2=None, width=None, radius=None,
            xs=None, length1=0, doStrFirst=1, ortho=True, ref=None):
        ...
        if not params['found'] or abs(params['solution']['angle']) < 0*m.pi:
            ...
            if Lfit >= 0:
                ...
                with nd.Cell('{}_{}'.format(name, xs), instantiate=self.instantiate, cnt=True) as ICcell:
                    ...
                    trace.trace_start()
                    if da1 != 0:
                        e1 = self._arc(radius=radius1, angle=da1, width=width, xs=xs).put()
                        self.line(La, width, xs=xs).put()
                    else:
                        e1 = self.line(La, width, xs=xs).put(0)
                    self._arc(radius=radius1, angle=m.degrees(A), width=width, xs=xs).put()
                    self.line(Lstr, width, xs=xs).put()
                    self._arc(radius=radius1, angle=-m.degrees(A), width=width, xs=xs).put()
                    self.line(Lb, width, xs=xs).put()
                    if da2 != 0:
                        self._arc(radius=radius1, angle=da2, width=width, xs=xs).put()
                    nd.Pin('a0', io=0, width=width, xs=xs).put(e1.pin['a0'])
                    nd.Pin('b0', io=1, width=width, xs=xs).put()
                    if arrow:
                        self.arrow.put(ICcell.pin['a0'])
            ...
            else: # solve start- or end-point connection with an extra bend
                if name is None:
                    name = 'ic_sbend_p2p'
                if doStrFirst == 0:
                    with nd.Cell(name, instantiate=self.instantiate, cnt=True) as ICcell:
                        ICcell.group = 'interconnect'
                        e1 = self.line(La, width, xs=xs).put()
                        self._arc(radius=radius1, angle=m.degrees(A), width=width, xs=xs).put()
                        nd.Pin('a0', io=0, width=width, xs=xs).put(e1.pin['a0'])
                        nd.Pin('b0', io=1, width=width, xs=xs).put()
                        if arrow:
                    ...
                    with nd.Cell(name, instantiate=self.instantiate, cnt=True) as ICcell:
                        ICcell.group = 'interconnect'
                        e1 = self.line(Lb, width, xs=xs).put()
    ...
    def _strt_bend_strt_p2p_solve(self, pin1=None, pin2=None, radius=None, xs=None, width=None):
        ...
        if abs(180-da) > 1e-5 and abs(180+da) > 1e-5:
            ...
            if sign(dy)*da < 0 or sign(dy)*da >= 180:
                found = False
                cfg.interconnect_errcnt += 1
                msg = "IC-{} strt_bend_strt: Wrong direction of end-point. Switching to bend_strt_bend in cell '{}' between {} and {}".\
                    format(cfg.interconnect_errcnt, cfg.cells[-1].cell_name, pin1.fxya(), pin2.fxya())
            elif  abs(g-90) < 1e-4:
                found = False
                cfg.interconnect_errcnt += 1
                msg = "IC-{} strt_bend_strt: pointers in-line. Switching to bend_strt_bend in cell '{}' between {} and {}".\
                    format(cfg.interconnect_errcnt, cfg.cells[-1].cell_name, pin1.fxya(), pin2.fxya())
            else:
                Ltap = 0
                ...
                if L1 >= 0 and L2 >= 0:
                    ...
                    found = False
                    cfg.interconnect_errcnt += 1
                    msg = "IC-{} strt_bend_strt: negative straight line. Switching to bend_strt_bend in cell '{}' between {} and {}".\
                    format(cfg.interconnect_errcnt, cfg.cells[-1].cell_name, pin1.fxya(), pin2.fxya())
        else:
            found = False
            cfg.interconnect_errcnt += 1
            msg = "IC-{} strt_bent_strt: angle not possible. Switching to bend_strt_bendin cell '{}' between {} and {}".\
                format(cfg.interconnect_errcnt, cfg.cells[-1].cell_name, pin1.fxya(), pin2.fxya())
        geo = [
            ('s', (L1, width)),
            ('b', (da, radius1, width)),
    ...
    def strt_bend_strt_p2p(self, pin1=None, pin2=None, radius=None, width=None,
        ...
        if params['found']:
            ...
            with nd.Cell('{}_{}'.format(name, xs), instantiate=self.instantiate, cnt=True) as ICcell:
                ICcell.group = 'interconnect'
                trace.trace_start()
                e1 = self.line(L1, width, xs=xs).put()
                ...
                if arrow:
                    self.arrow.put(ICcell.pin['b0'])
                    self.arrow.put(ICcell.pin['a0'], flip=pinflip)
                trace.trace_stop()
                ICcell.length_geo = trace.trace_length()
                ICcell.pin2 = pin2
            cfg.cp = pin1
    ...
    def _ubend_p2p_solve(self, pin1, pin2, length=0, xs=None, width=None,
        ...
        if abs(dy) < 2*radius1:
            d = -np.sign(dy) * (epsilon + radius1 - 0.5*abs(dy))
    ...
    def ubend_p2p(self, pin1=None, pin2=None, radius=None, width=None, xs=None,
        ...
        parse, params = self._ubend_p2p_solve(
            pin1, pin2, length=length, width=width, radius=radius,
            balance=balance, end_angle=end_angle)
        pin1, pin2, xs, width, _, radius1, radius2 = parse
...
""")
    ...
    def euler(self, pin=None, width=None, width2=None,
            ...
            name=None, arrow=True):
        """Create an Euler bend from a straight guide to curve radius at angle.
    ...
    def tube(self, geo, showpins=False, name=None, xs=None, arrow=True):
        ...
        with nd.Cell(name, instantiate=self.instantiate, cnt=True) as ICcell:
            ...
            for i, tube in enumerate(geo):
                if tube[0] == 's':
                    e1 = self.strt(length=tube[1][0], width=tube[1][1], arrow=False).put()
                elif tube[0] == 'b':
                    e1 = self.bend(angle=tube[1][0], radius=tube[1][1], width=tube[1][2], arrow=False).put()
                else:
                    cfg.interconnect_errcnt += 1
                    ...
                    ic_exception(msg)
                if i == 0:
                    nd.Pin('a0', io=0, width=geo[0][1][1], xs=xs).put(e1.pin['a0'])
            nd.Pin('b0', io=1, width=geo[-1][1][1], xs=xs).put()
            trace.trace_stop()
            ICcell.length_geo = trace.trace_length()
            if arrow:
                self.arrow.put(ICcell.pin['a0'])
    ...
    def connect(self, length=10, width1=None, width2=None, ic=None, name=None):
        ...
        Args:
            ic (Interconnect): interconnect to connect to
    ...
    def Tp_viper(self, x, y, w, width1=None, width2=None,
        xs=None, layer=None, N=200, epsilon=1e-6, name='viper', arrow=True,
        **kwargs):
        ...
        Tp = nd.Tp_viper(x=x, y=y, w=w, width1=width1, width2=width2,
            xs=xs, layer=layer, N=N, epsilon=epsilon, name=name, **kwargs)
